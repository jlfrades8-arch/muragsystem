...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="group relative overflow-hidden bg-gradient-to-br from-red-50 via-white to-rose-50 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 border border-red-100 hover:border-red-300 hover:-translate-y-1">
        ...
        <div class="relative p-6">
            ...
            <p class="text-4xl font-black text-gray-900 bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent">{{ $pets->whereIn('status', ['Pending', 'not yet rescue'])->count() }}</p>
...
<div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
    <div class="bg-gradient-to-r from-purple-50 via-pink-50 to-rose-50 p-6 border-b border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="text-xl font-bold text-gray-900 flex items-center">
                    <span class="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-600 rounded-lg flex items-center justify-center mr-3 shadow-lg">
                ...
                <p class="text-sm text-gray-600 mt-2 ml-13">Manage and update pet rescue statuses</p>
            </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Rescuer's Name</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gradient-to-r hover:from-purple-50/30 hover:to-pink-50/30 transition-all duration-200 group">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center justify-center w-8 h-8 bg-gradient-to-br from-purple-100 to-pink-100 text-purple-700 rounded-lg font-bold text-sm group-hover:scale-110 transition-transform">
                            {{ $index + 1 }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="text-sm font-semibold text-gray-900">{{ $pet['full_name'] ?? 'N/A' }}</span>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-700">{{ $pet['address'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $pet['location'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium">{{ $pet['condition'] ?? 'N/A' }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{{ $pet['kind'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $pet['color'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $pet['contact'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @php $s = $pet['status'] ?? 'Pending'; @endphp
                        @if($s === 'Pending' || $s === 'not yet rescue')
                        <button type="button"
                            class="mark-rescued-btn px-4 py-2.5 bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-red-500/30 hover:shadow-red-500/50 hover:scale-105"
                            data-pet-id="{{ $pet->id }}"
                            data-status="{{ $s }}">
                        ...
                        <button type="button"
                            class="mark-rescued-btn px-4 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-green-500/30 hover:shadow-green-500/50 hover:scale-105"
                            data-pet-id="{{ $pet->id }}"
                            data-status="{{ $s }}">
                            <span class="flex items-center">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button type="button"
                            disabled
                            class="px-4 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-xs font-bold rounded-xl opacity-60 cursor-not-allowed">
                            <span class="flex items-center">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                        ...
                        @elseif($s === 'Adopted')
                        <span class="px-4 py-2.5 inline-flex items-center text-xs font-bold rounded-xl bg-gradient-to-r from-purple-100 to-pink-100 text-purple-800 shadow-sm">
                        ...
                        @else
                        <span class="px-4 py-2.5 inline-flex items-center text-xs font-bold rounded-xl bg-gray-100 text-gray-800 shadow-sm">
                            {{ $s }}
                        </span>
                        @endif
                    </td>
    ...
    <div class="p-16 text-center bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        <h3 class="text-xl font-bold text-gray-800 mb-2">No Pets Reported Yet</h3>
        <p class="text-gray-600 mb-6">Start by creating your first pet rescue report</p>
        <a href="{{ route('rescue.form') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all shadow-lg hover:shadow-xl hover:scale-105">
