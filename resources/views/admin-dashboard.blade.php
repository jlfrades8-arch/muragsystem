    ...
    <div class="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 p-6 border-b border-gray-200">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                ...
                <a href="/dashboard" class="inline-flex items-center px-3 py-2 bg-white rounded-lg text-sm font-semibold text-gray-700 border border-gray-200 hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-3">
                            <div class="w-14 h-14 rounded overflow-hidden bg-gray-100 flex items-center justify-center">
                                @if(!empty($pet->image))
                                    <img src="{{ asset('storage/' . $pet->image) }}" alt="{{ $pet->pet_name ?? 'pet' }}" class="w-full h-full object-cover image-preview-{{ $pet->id }}">
                                @else
                                    <svg class="w-8 h-8 text-gray-300" fill="currentColor" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                                @endif
                            </div>

                            <div class="flex flex-col">
                                <label class="upload-label text-sm text-gray-700 font-medium cursor-pointer">
                                    <button type="button" class="upload-image-btn inline-flex items-center px-3 py-1.5 bg-white border border-gray-200 rounded text-xs font-semibold hover:bg-gray-50" data-pet-id="{{ $pet->id }}">Change Photo</button>
                                </label>
                                <input type="file" accept="image/*" class="hidden upload-input" data-pet-id="{{ $pet->id }}">
                            </div>
                        </div>
                    </td>
                        <span class="inline-flex items-center justify-center w-8 h-8 bg-gradient-to-br from-blue-100 to-purple-100 text-blue-700 rounded-lg font-bold text-sm group-hover:scale-110 transition-transform">
